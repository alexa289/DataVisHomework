...
SELECT f. title, COUNT(f.film_id) AS 'Rented_Movies'
FROM  film f
JOIN inventory i 
...
SELECT s.store_id, SUM(p.amount) AS 'Business in USD'
FROM payment p
JOIN staff s 
ON (p.staff_id=s.staff_id)
GROUP BY s.store_id
...
SELECT c.name AS 'Top 5 Genres', SUM(p.amount) AS 'GrossRevenue'
FROM category c
