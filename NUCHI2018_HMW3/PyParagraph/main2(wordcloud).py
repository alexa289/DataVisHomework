...
with open(txtinput, 'r') as file:
    ...
    total_spec_char = count1 + count2 + count3 + count4 + count5 + count6 + \
    ...
    print('Approximate letter count (per word): ',
          total_words_only / len(file_contents.split()))
    #Calculate the lenght of a sentence. Assuming each sentence ends in '.'
    print('Average sentence length (in words): ', len(
        file_contents.split())/file_contents.count('.'))
...
def create_wordcloud(text):
    twit_mask = np.array(Image.open(
        os.path.join(curdir, "twitter_mask.png")))

    stopword = set(STOPWORDS)

    wc = WordCloud(background_color = "white", mask = twit_mask, max_words = 600, stopwords = stopword)
    wc.generate(text)
    wc.to_file(os.path.join(curdir, "wc.png"))

create_wordcloud(file_contents)
